...
def extract_markdown_images(text:str) -> list[tuple[str,str]]:
    ...
    return re.findall(r"!\[([^\[\]]*)\]\(([^\(\)]*)\)", text) #Takes into account to register when an image is shown

def extract_markdown_links(text:str) -> list[tuple[str,str]]:
    '''gets the href and the url for the link given in plain text by given sequence predetermined by markwdown'''

    return re.findall(r"(?<!!)\[([^\[\]]*)\]\(([^\(\)]*)\)", text) #Takes into account to register when a link is shown
