...
def generate_pages_recursive(dir_path_content:str, template_path:str, dest_dir_path:str) -> None:
    ...
    for entry in list_entries:
        ...
        if os.path.isfile(path): #if file then copy everythign into the dest_dir from template
            ...
            generate_page(from_path=path, template_path=template_path, dest_path=dest_final_path) #generates page on given destination
        else:
            generate_pages_recursive(dir_path_content=path, template_path=template_path, dest_dir_path=dest_final_path)
