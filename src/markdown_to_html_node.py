...
def markdown_to_html_node( text:str) -> HTMLNODE:
    ...
    return HTMLNODE(tag="div", value= None, children= current_children, props= None)
...
def block_to_htmlnode(blocktype: BlockType, text: str) -> HTMLNODE:
    '''returns htmlnode from given blocktype and text'''

    if blocktype == BlockType.code: #check if blocktype is 
        code_block = HTMLNODE(tag="code", value=text[3:-3], children=None, props=None)
        return HTMLNODE(tag="pre", value=None, children=[code_block], props=None)
    
    elif blocktype == BlockType.unordered_list: #create tag for unorganized list
        cleaned_text = [x[2:] for x in text.split("\n")]
        list_lists = [HTMLNODE(tag='li', value=None, children=text_to_children(filtered)) for filtered in cleaned_text]
        return HTMLNODE(tag="ul", value=None, children=list_lists)

    elif blocktype == BlockType.ordered_list: #check ordered list
        cleaned_lists = []
        for i, block in enumerate(text.split("\n"), start=1):
            cleaned_block = block.replace(f"{i}. ", "")
            cleaned_lists.append(HTMLNODE(tag="li", value= None, children=text_to_children(cleaned_block)))
        return HTMLNODE(tag="ol", value=None, children=cleaned_lists)
        ...
        return HTMLNODE(tag=f"h{num_hash}", value=None, children=current_children)
        ...
        return HTMLNODE(tag="blockquote", value=None, children=current_children )

    else: #check qparagraphuote case
        cleaned_paragraph = " ".join([current for current in text.split("\n")])
        current_children = text_to_children(block=cleaned_paragraph) #get list of children from current text 
        return HTMLNODE(tag="p", value=None, children=current_children )
